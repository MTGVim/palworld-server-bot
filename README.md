...
- **기능**
  ...
  - 백준 랜덤 문제 출제 (`!랜덤 문제`, `!오늘의 문제`, solved.ac API 기반)
...
| `BOJ_SEARCH_API_URL` | 아니오 | `https://solved.ac/api/v3/search/problem` | 백준 문제 조회용 solved.ac 검색 API 엔드포인트 |
| `BOJ_FETCH_TIMEOUT_MS` | 아니오 | `5000` | 백준 문제 조회 타임아웃(ms) |
| `BOJ_TODAY_CACHE_PATH` | 아니오 | `/app/data/boj-today.json` | `!오늘의문제` 일일 고정 캐시 파일 경로 |
| `BOJ_TODAY_TIMEZONE` | 아니오 | `Asia/Seoul` | `!오늘의문제` 날짜 기준 타임존 |
...
- `!랜덤 문제 [쉬움|중간|어려움]` : 백준 랜덤 문제 1개 조회 (기본 `중간`, 한글 제목 문제만 출제)
- `!오늘의 문제 [(쉬움|중간|어려움)]` : 백준 오늘의 문제 1개 조회 (같은 난이도는 당일 고정, 기존 오늘의 문제 이력과 중복 제외, 기본 `중간`)
- `!오늘의 문제 리셋` : 오늘의문제 캐시 초기화 (관리자 전용, `ADMIN_USER_IDS` 대상)
