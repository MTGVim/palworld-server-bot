...
- `!가위바위보 <가위|바위|보>` : 봇과 가위바위보 1회 진행 (요청자 닉네임 기준 결과 표시, `✌️/✊/✋`, `💃/🥹/🤝`)
...
- `ℹ️ [rps][정보] 전적 저장소 준비 완료: ... path=... writable=... fileExists=... users=... games=...`
- `✅ [rps][정상] 전적 저장 마운트를 확인했습니다: type=... src=... dst=... rw=...`
- `✅ [rps][정상] 전적 저장 성공: path=... writes=... bytes=...`
- `🚨 [rps][경고] ...`이 나오면 바로 뒤 `🛠️ [rps][조치] ...` 안내를 그대로 적용한다.
