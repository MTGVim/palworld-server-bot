...
| `LEETCODE_GRAPHQL_URL` | 아니오 | `https://leetcode.com/graphql` | 리트코드 문제 조회 GraphQL 엔드포인트 |
| `LEETCODE_FETCH_TIMEOUT_MS` | 아니오 | `5000` | 리트코드 문제 조회 타임아웃(ms) |
| `LEETCODE_TODAY_CACHE_PATH` | 아니오 | `/app/data/leetcode-today.json` | `!오늘의문제` 일일 고정 캐시 파일 경로 |
| `LEETCODE_TODAY_TIMEZONE` | 아니오 | `Asia/Seoul` | `!오늘의문제` 날짜 기준 타임존 |
...
- `!랜덤문제 [쉬움|중간|어려움]` : 리트코드 랜덤 문제 1개 조회 (기본 `중간`)
- `!오늘의문제 [(쉬움|중간|어려움)]` : 리트코드 오늘의 문제 1개 조회 (같은 난이도는 당일 고정, 기본 `중간`)
